...
The $K$-sweep (Table~\ref{tab:ksweep}) shows what a \emph{capacity} limit looks like: everything solves $K{=}8$; the unarmed arms decline gracefully to $K{=}32$. The haystack task shows something categorically different (Table~\ref{tab:wall}); Figure~\ref{fig:wall} juxtaposes the two axes.
...
\begin{table}[t]
\centering
\caption{\textbf{The haystack wall} ($n{=}2$ seeds; $L{=}64$ haystack, 2 layers, proper budget; chance $\approx 0.019$). The same recurrences that store 32 competing pairs at $\ge 0.99$ fail at chance on \emph{4} pairs across a distractor haystack---already at the shortest tested length, for every transition type---while the (properly headed) attention control solves the task exactly.}
\label{tab:wall}
\vskip 0.1in
\small
\begin{tabular}{lc}
\toprule
arm ($L{=}64$, 2 layers) & accuracy \\
\midrule
attention (head dim 16) & \textbf{1.000} \\
\armed{} & 0.021 (chance) \\
\mambaref{} & 0.016 (chance) \\
\dgrank{} (RWKV-7) & 0.020 (chance) \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{It is a wall, not a dip.} The failure is already total at the \emph{shortest} tested length, inside the training distribution, at table-to-query gaps of only tens of tokens. A capacity limit degrades gracefully (as $K$ does); a decay account \citep{echo2026} predicts degradation that grows with distance---not chance at minimal distance. (A 1-layer sweep over $L{=}64$--$512$ was flat at chance but is excluded as under-capacity for this 2-hop task; the proper-depth wall is measured at $L{=}64$ only, so we make no claim about its length profile; a proper-depth length sweep is deferred to a future revision.)
\item \textbf{Transition richness does not help.} GDN's delta rule, Mamba's selectivity, and RWKV-7's removal key wall identically. Whatever is failing is not the state-transition's expressivity.
\item \textbf{Attention is untouched}, as expected: per-token KV entries cannot be overwritten by later writes.
\end{enumerate}

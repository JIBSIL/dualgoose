...
\textbf{The armed cell vs.\ real Mamba.} \armed{} at $0.983$ (10 seeds) clears the official fused Mamba-2 at matched state ($0.884$, measured in a separate session with the same probe) by ${\approx}{+}0.10$; within our harness it clears the under-trained \mambaref{} by $+0.39$. A paired, per-arm-tuned measurement of the official kernel in one session remains to be done (\S\ref{sec:threats}).
